/* ============================================================
   NURSE DASHBOARD  — v2 professional
      ...
      <tbody>
        ${d.todays_queue.length === 0
          ...
          <tr class="${e.has_allergy&&!e.has_vitals?'critical-row':''}" style="cursor:pointer" onclick="viewPatient(${e.patient_id})">
            ...
            <td>${e.has_allergy
              ? `<span class="badge badge-red" title="${e.allergies}"><i class="fa-solid fa-triangle-exclamation"></i> ${e.allergies.length>14?e.allergies.slice(0,14)+'…':e.allergies}</span>`
        ...
        <tbody>
          ${d.upcoming_followups.length === 0
            ...
            <tr style="cursor:pointer" onclick="viewPatient(${f.patient_id})">
              ...
              <td>${f.days_away===0?'<span class="badge badge-warn">Today</span>':
                f.days_away===1?'<span class="badge badge-warn">Tomorrow</span>':
        ...
        ${d.recent_patients.length===0
          ...
          : d.recent_patients.map(p=>{
              ...
              <tr style="cursor:pointer" onclick="viewPatient(${p.id})">
                ...
                <td>${ha?`<span class="badge badge-warn"><i class="fa-solid fa-triangle-exclamation"></i> ${p.allergies}</span>`:'<span class="badge badge-gray">None</span>'}</td>
...
async function submitQuickVitals(encounterId) {
  ...
  } else { toast('Failed to save vitals', 'error'); }
}
...
function showVitalsSearchModal() {
  ...
  <div class="search-bar" style="max-width:100%;margin-bottom:14px">
    ...
    <input type="text" id="vit-search" placeholder="Patient name or SmartID…" oninput="searchForVitals(this.value)">
  </div>
...
async function searchForVitals(q) {
  ...
  if (!el) return;
  if (!patients || !patients.length) { el.innerHTML='<p style="font-size:13px;color:var(--text3)">No patients found</p>'; return; }
  el.innerHTML = patients.slice(0,6).map(p=>`
    <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--border);gap:10px">
      <div>
        ...
        ${p.allergies&&p.allergies!=='None'?`<span class="badge badge-warn" style="margin-top:4px;font-size:10px"><i class="fa-solid fa-triangle-exclamation"></i> ${p.allergies}</span>`:''}
      </div>
...
async function startVitalsForPatient(patientId, patientName) {
  ...
  const res = await api('/api/encounters', {
    method:'POST',
    body: JSON.stringify({ patient:patientId, encounter_type:'OPD', visit_date:today,
  ...
  if (res && res.success) {
    ...
    else { closeModal(); toast('Encounter created — find patient in queue', 'success'); navigate('dashboard'); }
  } else { toast('Could not create triage encounter', 'error'); }
}
...
function startBatchTriage() {
  _batchQueue = (window._lastNurseData && window._lastNurseData.todays_queue || []).filter(e=>!e.has_vitals);
  _batchIdx = 0;
    ...
    `<div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;padding:10px 14px;background:var(--accent-light);border-radius:var(--radius-sm)">
       ...
       <div>
         ...
         <div style="font-size:12px;color:var(--text3)">${p.smart_id} · ${p.age} yrs · ${p.gender}
           ${p.has_allergy?'· <span style="color:var(--danger);font-weight:600">⚠ '+p.allergies+'</span>':''}</div>
       </div>
...
async function _saveBatch(encId) {
  ...
  if (!Object.values(payload).some(v=>v!==null&&v!=='')) { toast('Enter at least one vital sign','error'); return; }
  const res = await api(`/api/encounters/${encId}/vitals`,{method:'PATCH',body:JSON.stringify(payload)});
  if (res && res.success) { toast(`✓ Saved for ${_batchQueue[_batchIdx].patient_name}`,'success'); }
  _batchIdx++; _openBatchModal();
}
